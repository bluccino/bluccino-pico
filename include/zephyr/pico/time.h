...
static inline pi_us pi_preset(pi_us us)  // preset us clock time
{
  return *_pi_us_offset_() = _rtos_us_clock_() - us;
}

static inline pi_us pi_usec(void) // get current us-clock time
{
  pi_us us = _rtos_us_clock_();
  pi_us offset = *_pi_us_offset_();
  if (!offset) offset = pi_preset((pi_us)0);
  ...
  (int *ph, int *pmin, int *psec, int *pms, int *pus)
{
  ...
  pi_us us = pi_usec();   // clock time now in us
